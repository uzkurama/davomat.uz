...
<div class="card">
	<div class="card-content">
		<div style="margin-top: 100px;">
			...
			<?php
				$_SESSION['fak']= Yii::$app->user->identity->faculty_id;
				$fak=$_SESSION['fak'];
...
if (isset($_POST['save'])) {
$g_id = $_POST['st_gr'.'1'];
$st = mysqli_query($con,"SELECT * FROM `student` WHERE `group_id` = $g_id ORDER BY `group_id` ASC");
$count = mysqli_num_rows($st);
		$p = $_SESSION['para'];
	for ($i=1; $i<=$count; $i++ ){
		...
		$result = mysqli_query($con, "SELECT * FROM `attend` WHERE `lesson_id` = $p AND `student_id` = $st_id AND `date` LIKE '$date' LIMIT 1");

		if (mysqli_fetch_row($result)) {
			$msg = '<h4 style="color: red;">Davomat to\'ldirilgan</h4>';
		} else {
